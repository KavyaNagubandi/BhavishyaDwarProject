import React from "react";
import jobs from './Alljobs.json';
function Dashboard(){
    return (
        <div>
    <header>
        <div className="container-fluid bg-info">
            <div className="row">
                <div className="col-sm-2">{/*<!--<img src="./Bhavishya logo.png" alt="logo" width="100" height="100">-->*/}
                    <img src="data:image/png;base64," alt="logo" style={{width:"150px",height:"100px"}}></img>
                </div>
                <div className="col-sm-8">
                    
                    <nav className="navbar navbar-expand-sm navbar-nav-scroll sticky-top well position: fixed">
                        
                       
                        <button type="button" 
                        data-bs-toggle="collapse"
                        data-bs-target="#navbarNav"
                        aria-controls="navbarNav"
                        aria-expanded="false"
                        aria-label="Toggle navigation"
                        className="navbar-toggler">
                        <span className="navbar-toggler-icon"></span>
                        </button>
                    
                        <div className="collapse navbar-collapse justify-content between" id="navbarNav">
                        <ul className="navbar-nav mb-2 mb-lg-0">
                            <li className="nav-item px-2">
                                <br></br><a className="nav-link" href="/"><b style={{fontSize:"20px"}}>Home</b></a>
                            </li>
                            <li className="nav-item">
                                <br></br><a className="nav-link" href="Dashboard" ><b style={{fontSize:"20px"}}>AllJobs</b></a>
                            </li>
                            <li className="nav-item">
                                <br></br><a className="nav-link" href="Profile"><b style={{fontSize:"20px"}}>Profile</b></a>
                            </li>
                            <li className="nav-item">
                                <br></br><a className="nav-link" href="Notification"><b style={{fontSize:"20px"}}>Notification</b></a>
                            </li>
                            <li className="nav-item">
                                <br></br><a className="nav-link" href="Aboutus"><b style={{fontSize:"20px"}}>AboutUs</b></a>
                            </li>
                            <li className="nav-item">
                                <br></br><a className="nav-link" href="Contactus"><b style={{fontSize:"20px"}}>ContactUs</b></a>
                            </li>  
                        </ul>
                        </div>
                    </nav>
                </div>
                    <div className="col-sm-2">
                        <nav className="navbar navbar-expand-sm navbar-nav-scroll position: fixed">
                            <ul className="navbar-nav">
                                <li className="nav-item">
                                   <br></br><a className="btn btn-outline-dark" type="button" href="/"><b style={{fontSize:"17px"}}>LogOut</b></a>
                                </li>
                            </ul>
                        </nav>
                    </div>                
             </div>
        </div>
    </header>
    
    <div className="container-fluid" id="Alljobs">
        <div className="row">
          <div className="col-sm-9">
            <ul className="nav nav-tabs" role="tablist">
                <li className="nav-item">
                  <a className="nav-link active " data-bs-toggle="tab"  href="#all">All Jobs</a>
                </li>
                <li className="nav-item">
                  <a className="nav-link"  data-bs-toggle="tab"  href="#fulltime">Full Time Jobs </a>
                </li>
                <li className="nav-item">
                  <a className="nav-link"  data-bs-toggle="tab"  href="#parttime">Part time Jobs </a>
                </li>
                
             
                <li className="nav-item">
                    <input type="text"  data-bs-toggle="tab" className="form-control" placeholder="Search"></input>
                </li>
            </ul>
            <br></br>
            
           <div className="container border"><br></br>
            <div className="tab-content">
                <div  className="container tab-pane active" id="all">
                    <div className="table-responsive">
                        <table className="table border-dark">
                            <tbody className="table-warning">{
                                jobs && jobs.map((record)=>{
                                   return(
                                    <tr key={record.id}>
                                        <td><img src={record.image} alt="Google" style={{width:"150px",height:"100px"}} ></img> </td>
                                        <td><b>{record.companyname}</b><br></br>
                                           <i> <strong>{record.work}</strong></i>
                                        </td>
                                        <td>
                                        <button type="button" className="btn btn-info" id="googleinfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="googleapply">Apply</button> 
                                    </td>
                                    </tr>
                                   );
                                })
                            }   {/*<tr>
                                    <td><img src="https://play-lh.googleusercontent.com/1-hPxafOxdYpYZEOKzNIkSP43HXCNftVJVttoo4ucl7rsMASXW3Xr6GlXURCubE1tA=w3840-h2160-rw" alt="Google" style={{width:"150px",height:"100px"}} ></img></td>
                                    <td><b>Data Base Development</b><br></br>
                                        <p>google</p>
                                    </td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="googleinfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="googleapply">Apply</button> 
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,f_auto,q_auto:eco,dpr_1/v1415386231/utypaslbyxwfuwhfdzxd.png"  alt="infosys" style={{width:"150px",height:"100px"}} ></img></td>
                                    <td>
                                        <b>Sr.Associate/Specialist </b><br></br>
                                        <p>Infosys</p>
                                    </td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="infosysinfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="infosysapply">Apply</button> 
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOvnVyeMo22zO48NlNNPFHR4pKvCODEEKiAw&usqp=CAU" alt="Microsoft"style={{width:"150px",height:"100px"}} ></img></td>
                                    <td>
                                        <b>MIS Executive</b><br></br>
                                        <p>Microsoft</p>
                                    </td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="microsoftinfo">View More</button> <br></br><br></br>
                                        <button type="button" className="btn btn-success" id="microsoftapply">Apply</button> 
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="https://i.pinimg.com/originals/d7/25/2a/d7252a71a8943948bb9f438aa6fcd6bf.jpg" alt="amazon"style={{width:"150px",height:"100px"}} ></img></td>
                                    <td>
                                        <b>Database Administrator</b><br></br>
                                        <p>Amazon</p>
                                    </td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="amazoninfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="amazonapply">Apply</button> 
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="https://1000logos.net/wp-content/uploads/2021/04/Oracle-logo.png" alt="oracle"style={{width:"150px",height:"100px"}} ></img></td>
                                    <td>
                                        <b>Database Developer</b><br></br>
                                        <p>Oracle</p>
                                    </td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="oracleinfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="oracleapply">Apply</button> 
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="https://1000logos.net/wp-content/uploads/2016/10/Apple-Logo.png" alt="apple"style={{width:"150px",height:"100px"}} ></img></td>
                                        <td><b>Designer</b><br></br>
                                        <p>Apple</p></td>
                                    <td>
                                        <button type="button" className="btn btn-info" id="appleinfo">View More</button> <br></br> <br></br>
                                        <button type="button" className="btn btn-success" id="appleapply">Apply</button> 
                                    </td> 
                        </tr>*/}
                        </tbody>
                    </table>
                    </div>          
                </div>
            
            <div   id="fulltime" className= "container tab-pane fade">
                <div className="table-responsive">
                    <table className="table border-dark">
                        <tbody className="table-danger">
                            <tr>
                                <td><img src="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,f_auto,q_auto:eco,dpr_1/v1415386231/utypaslbyxwfuwhfdzxd.png"  alt="infosys"style={{width:"150px",height:"100px"}}  ></img></td>
                                <td><b>Database Developer </b><br></br>
                                    <p>Infosys</p></td>
                                <td><button type="button" className="btn btn-info" id="infosysinfo">View More</button> <br></br> <br></br>
                                    <button type="button" className="btn btn-success" id="infosysapply">Apply</button> 
                                </td>  
                            </tr>
                            <tr>
                                <td><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOvnVyeMo22zO48NlNNPFHR4pKvCODEEKiAw&usqp=CAU" alt="Microsoft"style={{width:"150px",height:"100px"}} ></img></td>
                                <td>
                                    <b>MIS Executive</b><br></br>
                                    <p>Microsoft</p>
                                </td>
                                <td>
                                    <button type="button" className="btn btn-info" id="microsoftinfo">View More</button> <br></br> <br></br>
                                    <button type="button" className="btn btn-success" id="microsoftapply">Apply</button> 
                                </td>
                            </tr>
                            <tr>
                                <td><img src="https://i.pinimg.com/originals/d7/25/2a/d7252a71a8943948bb9f438aa6fcd6bf.jpg" alt="amazon"style={{width:"150px",height:"100px"}} ></img></td>
                                <td>
                                    <b>Database Administrator</b><br></br>
                                    <p>Amazon</p>
                                </td>
                                <td>
                                    <button type="button" className="btn btn-info" id="amazoninfo">View More</button> <br></br> <br></br>
                                    <button type="button" className="btn btn-success" id="amazonapply">Apply</button> 
                                </td>
                            </tr>
                        </tbody>
                   </table>
                </div>
           </div>
           <div  className="container tab-pane fade" id="parttime">
            <div className="table-responsive">
                <table className="table border-dark">
                    <tbody className="table-info">
                        <tr>
                            <td><img src="https://play-lh.googleusercontent.com/1-hPxafOxdYpYZEOKzNIkSP43HXCNftVJVttoo4ucl7rsMASXW3Xr6GlXURCubE1tA=w3840-h2160-rw" alt="Google"style={{width:"150px",height:"100px"}}  ></img></td>
                            <td><b>Data Base Development</b><br></br>
                                <p>google</p>
                            </td>
                            <td>
                                <button type="button" className="btn btn-info" id="googleinfo">View More</button> <br></br> <br></br>
                                <button type="button" className="btn btn-success" id="googleapply">Apply</button> 
                            </td>
                        </tr>
                        <tr>
                            <td><img src="https://1000logos.net/wp-content/uploads/2021/04/Oracle-logo.png" alt="oracle"style={{width:"150px",height:"100px"}} ></img></td>
                            <td><b>MIS Executive</b><br></br>
                                <p>Oracle</p></td>
                            <td>
                                <button type="button" className="btn btn-info" id="oracleinfo">View More</button> <br></br> <br></br>
                                <button type="button" className="btn btn-success" id="oracleapply">Apply</button> 
                            </td> 
                        </tr>
                        <tr>
                            <td><img src="https://1000logos.net/wp-content/uploads/2016/10/Apple-Logo.png" alt="apple"style={{width:"150px",height:"100px"}} ></img></td>
                            <td><b>Designer</b><br></br>
                                <p>Apple</p></td>
                            <td>
                                <button type="button" className="btn btn-info" id="appleinfo">View More</button> <br></br> <br></br>
                                <button type="button" className="btn btn-success" id="appleapply">Apply</button> 
                            </td> 
                        </tr>
                    </tbody>
               </table>
            </div>
       </div>
     </div>
        
    </div>
       </div>
         <div className="col-sm-3">
            <b>Jobs Based on</b>
            <form> 
                <label for="IML">
                    <br></br>
                    <input type="checkbox" value="IML" name="subject" id="201"></input>IML
                </label>
                <br></br>
                <label for="Cyber Security">
                    <input type="checkbox" value="Cyber Security" name="subject" id="202"></input>Cyber Security
                </label>
                <br></br>
                <label for="Block Chain">
                    <input type="checkbox" value="Block Chain" name="subject" id="203"></input>Block Chain
                </label>
                <br></br>
                <label for="MongoDB">
                    <input type="checkbox" value="MongoDB" name="subject" id="204"></input>MongoDB
                </label>
                <br></br>
                <label for="Database">
                    <input type="checkbox" value="Database" name="subject" id="205"></input>Data Base
                </label>
            </form>
          </div>
        </div>
    </div>
    
    <footer className="footer">
        <div className="container-fluid bg-dark">
            <hr></hr>
            <h1 style={{color:"aliceblue"}}> Bhavishya Dwar </h1>
            <div className="container">
                <p style={{color:"blue"}}>
                    <a className="nav-link" href="Aboutus">About Us</a>
                    <a className="nav-link" href="Contactus">Contact Us</a>
                    <br></br>
                </p>
                <p style={{color:"aliceblue"}}></p>
            </div>
        </div>
    </footer>
    
</div>

    );
}
export default Dashboard ;